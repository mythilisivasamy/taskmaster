...
private uri='http://localhost:8000/api'; 
  ...
  getTasks():Observable<any>{
    return  this.http.get(`${this.uri}/task`)
    .pipe( tap(tasks => this.tasks$!=of(tasks)),
    ...
    addTask(task:Task):Observable<unknown>{
      ...
      editTask(task:Task):Observable<unknown>{
        return this.http.put<Task>(`${this.uri}/task/${task._id}`,task!,this.httpOptions)
        .pipe(catchError(this.handleError))
      }
      
      deleteTask(task:Task):Observable<unknown>{
         return this.http.delete<Task>(`${this.uri}/task/${task._id}`,this.httpOptions)
        .pipe(catchError(this.handleError))
      }
    }